...
class medical_surgery(models.Model):
    ...
    anesthetist_physician_id = fields.Many2one(
        'medical.physician', 'Anesthetist')
    operating_room_id = fields.Many2one(
        'medical.hospital.oprating.room', 'Operating Room')
    ...
    @api.constrains('date', 'surgery_end_date')
    def check_dates(self):
        for rec in self:
            if rec.date > rec.surgery_end_date:
    ...
    def onchange_duration(self):
        if self.date and self.surgery_end_date:
            ...
            d2 = datetime.strptime(str(self.surgery_end_date), "%Y-%m-%d %H:%M:%S")
            rd = d2 - d1
            houser = rd // 3600
            val = str((rd.seconds // 3600) % 3600)
            self.surgery_length = str(
                rd.seconds // 3600) + '' + 'h' + ' ' + str((rd.seconds // 60) % 60) + '' + 'm'
    ...
    def create(self, val):
        for vals in val:
            if vals.get('operating_room_id') and vals.get('date'):
                surgery_room = self.search([('operating_room_id', '=', vals.get('operating_room_id')), (
                    'date', '<', vals.get('date')), ('surgery_end_date', '>', vals.get('date'))])
                if surgery_room:
                    raise ValidationError(
                        'Surgery room not available in this Time.')
        return super(medical_surgery, self).create(val)
