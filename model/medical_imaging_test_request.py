...
class medical_imaging_test_request(models.Model):
    ...
    def create(self, vals_list):
        ...
        result = super(medical_imaging_test_request, self).create(vals_list)
        return result
    ...
    def done(self):
        ...
        request_id = self.env['medical.imaging.test.request'].search(
            [('name', '=', request)])
        ...
        if new_created_id_list:
            ...
            action = self.env.ref(
                'healthcare_management.action_medical_imaging_test_result')
            list_view_id = imd.sudo()._xmlid_to_res_id(
                'healthcare_management.medical_imaging_test_result_view')
