...
class medical_patient_lab_test(models.Model):
    ...
    pathologist_id = fields.Many2one('medical.pathology.group', string='Pathologist')
    prescription_id = fields.Many2one('medical.prescription.order', string='Prescription')
    policy_no = fields.Char('Policy Number')

    @api.model_create_multi
    def create(self, val):
        result = super(medical_patient_lab_test, self).create(val)
        for vals in val:
    ...
    def create_lab_test(self):
        ...
        for browse_record in self:
            ...
            if res_ids:                     
                ...
                action = self.env.ref('healthcare_management.action_medical_lab_form')
                list_view_id = imd.sudo()._xmlid_to_res_id('healthcare_management.medical_lab_tree_view')
                form_view_id  =  imd.sudo()._xmlid_to_res_id('healthcare_management.medical_lab_form_view')
                result = {
                                'name': action.name,
                                'help': action.help,
                                'type': action.type,
