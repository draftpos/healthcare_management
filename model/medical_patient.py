...
class medical_patient(models.Model):
    ...
    def print_report(self):
        return self.env.ref('healthcare_management.report_print_patient_card').report_action(self)
    ...
    patient_name = fields.Char('Patient Name', compute='_compute_patient_name', store=True)
    name = fields.Char(string='ID', readonly=True, copy=False, default=lambda self: _('New'))
    ...
    receivable = fields.Float(string="Receivable", compute="_compute_actual_receivable")
    ...
    smoking_number = fields.Integer(string="Cigarettes per day")
    ...
    medication_ids = fields.One2many('medical.patient.medication', 'medical_patient_medication_id')
    ...
    image_ids = fields.One2many('medical.imaging.test.result', 'patient_id', string='Images')
    pulse_rate = fields.Integer('Pulse Rate')
    oxygen_rate = fields.Integer('Oxygen Rate')
    blood_pressure = fields.Integer('Blood Pressure')
    weight = fields.Float('Weight(KG)')
    patient_attachement_ids = fields.One2many('medical.patient.attachment', 'patient_id', string='Attachments')

    def _compute_actual_receivable(self):
        for record in self:
            if record.patient_id:
                invoice_ids = self.env['account.move'].search([('partner_id', '=', record.patient_id.id)])
                if invoice_ids:
                    record.receivable = sum(invoice_ids.mapped('amount_residual')) if invoice_ids.mapped('amount_residual') else 0.00
                else:
                    record.receivable = 0.00
            else:
                record.receivable = 0.00
    ...
    def write(self, vals):
        for record in self:
            if vals.get('patient_name') and record.patient_id:
                record.patient_id.write({'name': vals.get('patient_name')})
        return super(medical_patient, self).write(vals)
