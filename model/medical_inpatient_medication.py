...
class medical_inpatient_medication(models.Model):
    ...
    medical_medicament_id = fields.Many2one(
        'medical.medicament', string='Medicament', required=True)
    ...
    medical_inpatient_medication_physician_id = fields.Many2one(
    ...
    medical_drug_route_id = fields.Many2one(
        'medical.drug.route', string=" Administration Route ")
    dose = fields.Float(string='Dose')
    qty = fields.Integer(string='X')
    medical_dose_unit_id = fields.Many2one(
        'medical.dose.unit', string='Dose Unit')
    ...
    medical_medication_dosage_id = fields.Many2one(
    ...
    medical_inpatient_registration_id = fields.Many2one(
        'medical.inpatient.registration', string='Medication')
    inpatient_admin_times_ids = fields.One2many(
        'medical.inpatient.medication.admin.time', 'medical_inpatient_admin_time_medicament_id', string='Admin')
    inpatient_log_history_ids = fields.One2many(
        'medical.inpatient.medication.log', 'medical_inaptient_log_medicament_id', string='Log History')

    @api.constrains('start_treatment', 'end_treatment')
    def check_dates(self):
        for rec in self:
            if rec.start_treatment > rec.end_treatment:
