...
class create_prescription_shipment(models.TransientModel):
    ...
    def create_prescription_shipment(self):
        ...
        if priscription_record.is_shipped == True:
            raise ValidationError('All ready shipped.')
        res = sale_order_obj.create({'partner_id':priscription_record.patient_id.patient_id.id , 'prescription_id': priscription_record.id})
        if priscription_record.prescription_line_ids:
            ...
            raise ValidationError('There is no shipment line.')
