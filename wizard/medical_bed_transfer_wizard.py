...
class medical_bed_transfer_wizard(models.TransientModel):
    ...
    _description = "Medical Bed Transfer Wizard"

    medical_hospital_bed_id = fields.Many2one(
        'medical.hospital.bed', string="New Bed", required=True)
    reason = fields.Char('Reason', required=True)

    def bed_transfer(self):
        record = self
        medic_imp_obj = self.env['medical.inpatient.registration']
        medic_imp_rec = medic_imp_obj.browse(self.env.context.get('active_id'))
        if medic_imp_rec.state == 'hospitalized':
            if record.medical_hospital_bed_id.state == 'free':
                ...
                medic_imp_rec.write({'medical_hospital_bed_id': record.medical_hospital_bed_id.id,
                                     'bed_transfers_ids': [(0, 0, {
                                         'date': date.today(),
            ...
            raise UserError(
                "Bed transfer is only allowed in hospitalize stage")
