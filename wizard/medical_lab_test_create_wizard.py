...
class medical_lab_test_create(models.TransientModel):
    ...
    def create_lab_test(self):
        ...
        browse_records = lab_rqu_obj.browse(self.env.context.get('active_ids'))
        result = {}
        for browse_record in browse_records:
            ...
            if res_ids:
                ...
                write_ids = lab_rqu_obj.browse(self.env.context.get('active_id'))
                write_ids.write({'state': 'tested'})
                action = self.env.ref('healthcare_management.action_medical_lab_tree')
